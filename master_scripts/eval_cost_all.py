...
def plot_eco_effect(x,y, **kwargs):
    points = plt.scatter(x,y, s=190, linewidth=0, edgecolors='k', **kwargs)
...
stats['label'] = stats.index.values
idx = stats.label.str.contains('region18')
stats_sub = stats[idx.values]
stats_sub['msr_type'] = stats_sub.label.str.split('_', expand=True).iloc[:,0]
stats_sub.drop(['relocation_region18_p20', 'relocation_region18_p80'], inplace=True)
hue_order = stats_sub.msr_type.unique()
markers = ['o', 's', 'd', 'D', 'H', 'h' ]
...
g.map_lower(plt.scatter, s=90)
#g.map_diag(plt.hist, bins=20)
g.add_legend(prop={'size': 16})
...
#pp30 = pareto_points(stats_sub.loc[:,['cost_sum', 'dwl_lower_area']])
#pp30.plot('cost_sum', 'dwl_lower_area', ax=g.axes[3,0], label='opt')
#pp31 = pareto_points(stats_sub.loc[:,['dFI', 'dwl_lower_area']])
#pp31.plot('dFI', 'dwl_lower_area', ax=g.axes[3,1], label='opt')

mpld3.save_html(plt.gcf(), 'matrix_subLocs_all.html')
plt.savefig('matrix_owner_all.png', dpi=150)
#%%
idx = pd.Series(delta_wl.columns.values).str.split('_', expand=True).loc[:,1] == 'region18'
delta_wl_sub = delta_wl.loc[:,idx.values]
delta_wl_sub.drop(['relocation_region18_p20', 'relocation_region18_p80'], inplace=True, axis=1)
sns.set_style('ticks')
col_order = delta_wl_sub.min().sort_values(ascending=False).index.values
delta_wl_sub[col_order].plot.line(colormap='viridis_r', linewidth=2)
plt.xlim(867.5, 883)
plt.ylim(-0.17, 0.02)
plt.legend(fontsize=10,loc='lower_right')
plt.title('Waterstandsdaling', fontsize=15)
plt.savefig('Gendt_delta_wl.png', dpi=300)
## BIOSAFE
