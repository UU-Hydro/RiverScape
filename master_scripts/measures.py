...
def write_measure(msr, msr_root_dir):
    ...
    msr_dir_name = string.join([settings['msr_type'], settings['ID']], sep='_')
...
#            write_measure(msr, msr_root_dir)
    ...
    ###########################################################################
    # Apply favorite measure per owner type
    
    #- Make masks
    owners = pcr.readmap(os.path.join(restraints_dir, 'owner_type.map'))
    mask_rws = owners == pcr.nominal(1) # Rijkswaterstaat
    mask_wtb = owners == pcr.nominal(2) # waterschap
    mask_mun = owners == pcr.nominal(3) # municipality
    mask_com = owners == pcr.nominal(4) # commercial company
    mask_cit = owners == pcr.nominal(5) # citizens
    mask_sbb = owners == pcr.nominal(6) # staatsbosbeheer
    mask_prv = owners == pcr.nominal(7) # province
    mask_pvn = owners == pcr.nominal(8) # province nature
    mask_fdt = owners == pcr.nominal(9) # foundation
    mask_sgc = owners == pcr.nominal(10) # sand, gravel, clay companies
    mask_oth = owners == pcr.nominal(11) # other, mostly churches
    mask_evr = pcr.boolean(1)            # every_where
    
    settings_ID = ['smooth', 'natural']
    owner_labels = ['rws', 'wtb', 'mun', 'com', 'cit',
                    'sbb', 'prv', 'prn', 'fdt', 'sgc',
                    'evr']
    owner_masks = [mask_rws, mask_wtb, mask_mun, mask_com, mask_cit,
                   mask_sbb, mask_prv, mask_pvn, mask_fdt, mask_sgc,
                   mask_evr]
    
    #- Generate six measures per owner with two ecotopes
    msr_root_dir = os.path.join(output_dir, 'measures_ensemble03/maps')
    pcrRecipes.make_dir(msr_root_dir)
    for label_owner, mask_owner in zip(owner_labels, owner_masks):
        msr_list = []
        ID = label_owner + '_smooth'
        print ID
        msr_groynes = waal_msr.groyne_lowering_msr(settings_smooth, mask=mask_owner, ID=ID)
        msr_dikera = waal_msr.main_dike_raising_msr(settings_smooth, mask=mask_owner, ID=ID)
        msr_minemb = waal_msr.minemb_lowering_msr(settings_smooth, mask=mask_owner, ID=ID)
        msr_list.append([msr_groynes, msr_dikera, msr_minemb])
        for ii, settings in enumerate([settings_smooth, settings_natural]):
            ID = label_owner + '_' + settings_ID[ii]
            print ID
            msr_smo = waal_msr.smoothing_measure(settings, mask=mask_owner, ID=ID)
            msr_low = waal_msr.lowering_measure(settings, mask=mask_owner, ID=ID)
            msr_ch = waal_msr.side_channel_measure(settings, mask=mask_owner, ID=ID)
            msr_ch = msr_ch.mask_out(mask_owner)
            
            msr_list.append([msr_smo, msr_low, msr_ch])
        for container in msr_list:
            for msr in container:
                write_measure(msr, msr_root_dir)
    end_time = time.time()
    print 'Processing time in seconds: ', end_time - start_time
    
    #- calculate owner area
    terrestrial = lulc.sections > 1
    owners_terr = pcr.ifthen(terrestrial, owners)
    df = pd.DataFrame(data={'ot': pcr.pcr2numpy(owners_terr, 0).flatten(),
                            'area':1})
    area = df.groupby('ot').count() # area in cells
    owner_types = ['nan','rws', 'waterboard', 'municipality', 'company', 'citizens',\
                   'state_forest', 'province', 'prvc_nature', 'foundation', 
                   'sand_clay', 'other']
    area['owners'] = owner_types
    area['area_ha'] = (area.area * 625 / 1e4).astype('int')
    area.drop(0, axis=0, inplace=True)
    print area.sort_values('area')
    print area.area_ha.sum()
