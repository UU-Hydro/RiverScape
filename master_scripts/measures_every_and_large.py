...
def write_measure(msr, msr_root_dir):
    ...
    msr_dir_name = string.join([msr.settings['msr_type'], msr.settings['ID']],
                               sep='_')
    ...
    with open(os.path.join(msr_dir_path, 'settings.txt'), 'w') as outfile:
        for k,v in msr.settings.iteritems():
            outfile.write('%s:%s\n' % (k,v))
...
#        pcr.aguila(dem)
    ...
    def smoothing_measure(self, settings, mask=None, ID='dummy-ID'):
        """
        Create a Measure class for roughness lowering.
        """
        self.settings = settings
...
#            write_measure(msr, msr_root_dir)
    ...
    mask_lrg = pcr.pcror(mask_rws, mask_sbb) # largest state-based owners
    
    settings_ID = ['smooth', 'natural']
    owner_labels = ['lrg', 'evr']
    owner_masks = [mask_lrg, mask_evr]
    
    #- Generate six measures per owner with two ecotopes
    msr_root_dir = os.path.join(output_dir, 'measures_ensemble04/maps')
    pcrRecipes.make_dir(msr_root_dir)
    for label_owner, mask_owner in zip(owner_labels, owner_masks):
        ...
        write_measure(msr_groynes, msr_root_dir)
        
        msr_dikera = waal_msr.main_dike_raising_msr(settings_smooth, mask=mask_owner, ID=ID)
        write_measure(msr_dikera, msr_root_dir)
        
        msr_minemb = waal_msr.minemb_lowering_msr(settings_smooth, mask=mask_owner, ID=ID)
        write_measure(msr_minemb, msr_root_dir)

        for ii, settings_msr in enumerate([settings_smooth, settings_natural]):
            ID = label_owner + '_' + settings_ID[ii]
            print '\t', ID
            msr_smo = waal_msr.smoothing_measure(settings_msr, mask=mask_owner, ID=ID)
            write_measure(msr_smo, msr_root_dir)

            msr_low = waal_msr.lowering_measure(settings_msr, mask=mask_owner, ID=ID)
            write_measure(msr_low, msr_root_dir)
            
            msr_ch = waal_msr.side_channel_measure(settings_msr, mask=mask_owner, ID=ID)
            msr_ch = msr_ch.mask_out(mask_owner)
            write_measure(msr_ch, msr_root_dir)
    end_time = time.time()
